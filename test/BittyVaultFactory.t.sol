...
contract BittyVaultFactoryTest is Test {
    ...
    function test_FactoryRevertsIfAddressZero() public {
        ...
        factory.initialize(address(0), wethAddress, whiteListAddress, migratorAddress);

        vm.expectRevert(AddressZero.selector);
        factory.initialize(vaultImplementation, address(0), whiteListAddress, migratorAddress);
    }

    function test_InitializeCanHaveZeroWhiteListAndMigrator() public {
        factory.initialize(vaultImplementation, wethAddress, address(0), address(0));
    ...
    function test_DeployVaultRevertsIfAddressNotWhiteListed() public {
        factory.initialize(vaultImplementation, wethAddress, whiteListAddress, migratorAddress);
