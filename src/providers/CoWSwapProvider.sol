...
contract CoWSwapProvider is IIntentProvider, IERC1271, Ownable, Initializable {
     ...
     *      NOTE: Cannot be used with AssetManager.rebalance() as-is - settlement is async.
    ...
    function trade(bytes memory data) external payable override onlyOwner {
        ...
        if (sellToken != address(0)) {
            ...
            IERC20(sellToken).safeApprove(vaultRelayer, sellAmount);
        }
    ...
    /**
     * @notice Compute order UID for a given order
     * @param order The CoW Protocol order
     */
