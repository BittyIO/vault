 ...
 *         operator (set via {IBittyV1Owner.setOperator}) — stored pending until the owner approves it
 *         (the approve* / approveSend functions on {IBittyV1Owner}). Implemented by {BittyV1Vault}.
 */
interface IBittyV1Operator {
    ...
    /**
     * @notice Owner: execute a batch of transfers immediately. Operator: queue the entire batch for
    ...
    /**
     * @notice Owner, or the operator who proposed it: cancel a pending one-off send.
     */
